...
cd = pjoin(config.connect + '-centered/')

def all_participant_matrix(connectivity_directory):
    out_dir = pjoin(config.figures, 'matrices-centered')
    base_out_dir = pjoin(config.figures, 'matrices-centered', 'base')
    early_out_dir = pjoin(config.figures, 'matrices-centered', 'early')
    late_out_dir = pjoin(config.figures, 'matrices-centered', 'late')
    os.makedirs(out_dir, exist_ok=True)
    os.makedirs(base_out_dir, exist_ok=True)
    os.makedirs(early_out_dir, exist_ok=True)
    os.makedirs(late_out_dir, exist_ok=True)
    
    #subid = ['sub-01', 'sub-02', 'sub-03', 'sub-05', 'sub-06',  'sub-07',  'sub-10',  'sub-11',  'sub-12',  'sub-13',   'sub-14',   'sub-15',   'sub-16',  'sub-17',  'sub-18',  'sub-19', 
    ...
    for i in all_subjects_cmat_data:
        if i[-9:] == "_base.tsv":
            ...
            plt.savefig(pjoin("/ssd1920/lamplab/projects/stat/statisticalmanifolds/figures/fig-components-pngs-k3/matrices-centered/base/", i[122:128]))
           #plt.savefig(f"/ssd1920/lamplab/projects/stat/statisticalmanifolds/figures/fig-components-pngs-k3/matrices/base/{i[113:120]}_connectivity.png")
            ...
            plt.savefig(pjoin("/ssd1920/lamplab/projects/stat/statisticalmanifolds/figures/fig-components-pngs-k3/matrices-centered/early/", i[122:128]))
        elif i[-9:] == "_late.tsv":
            df = pd.read_table(i)
            df = df.drop([0], axis=0)
            df = df.drop(['Unnamed: 0'], axis=1)
            ...
            plt.savefig(pjoin("/ssd1920/lamplab/projects/stat/statisticalmanifolds/figures/fig-components-pngs-k3/matrices-centered/late/", i[122:128]))
all_participant_matrix(cd)

#mean_base_connectivity = np.mean([base_cmats[0], base_cmats[1], base_cmats[3], base_cmats[4], base_cmats[5], base_cmats[6], base_cmats[7], base_cmats[8], base_cmats[9], base_cmats[10], base>
#early_base_connectivity = np.mean([early_cmats[0], early_cmats[1], early_cmats[3], early_cmats[4], early_cmats[5], early_cmats[6], early_cmats[7], early_cmats[8], early_cmats[9], early_cmat>
#late_base_connectivity = np.mean([late_cmats[0], late_cmats[1], late_cmats[3], late_cmats[4], late_cmats[5], late_cmats[6], late_cmats[7], late_cmats[8], late_cmats[9], late_cmats[10], late>
...
plt.savefig(f"/ssd1920/lamplab/projects/stat/statisticalmanifolds/figures/fig-components-pngs-k3/matrices-centered/base/mean_connectivity.png")

ax = sns.heatmap(early_base_connectivity, square=True, vmin=-.8, vmax=.8, center=0, cmap=sns.diverging_palette(220, 20, n=200))
plt.savefig(f"/ssd1920/lamplab/projects/stat/statisticalmanifolds/figures/fig-components-pngs-k3/matrices-centered/early/mean_connectivity.png")
