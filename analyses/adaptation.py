...
from utils import parse_roi_names, get_surfaces, test_regions
from plotting import plot_cbar, yeo_cmap, weights_to_vertices, plot_3d, set_plotting, get_sulc, stat_cmap, pairwise_stat_maps
...
def anova_stat_map(anova, out_dir, outline=True):
    ...
    cmap = 'autumn'
    #cmap = cmr.get_sub_cmap(stat_cmap(), .5, 1)
    cmap_min = vmin / vmax
    ...
    surfaces = get_surfaces()
    sulc = get_sulc()
    x = weights_to_vertices(fvals, Config().atlas, df['roi_ix'].values)
