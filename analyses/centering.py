...
from plotting import set_plotting
...
def embedding_plot(data, out_dir):
    for d, name in zip(data, ['uncentered', 'centered']):
        fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(3.5, 2.5), sharey=True, sharex=True)
        kwargs = dict(legend=False, s=10, clip_on=False)
        sns.scatterplot(x=0, y=1, hue='sub', data=d, ax=axes[0], **kwargs)
        sns.scatterplot(x=0, y=1, hue='epoch', data=d, ax=axes[1], palette='Dark2', **kwargs)
        for ax in axes:
...
def main():
    config = Config()
    original = get_files([config.connect, 'sub*/*full*.tsv'])
    centered = get_files([config.dataset_dir, 'connectivity-centered/sub*/*full*.tsv'])
